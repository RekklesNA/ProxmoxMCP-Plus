...
startup rather than silently weakening a denylist. An empty allowlist exposes no tools,
while an empty denylist preserves the complete available surface. Filtering occurs before
MCP registration, so hidden schemas are absent from `tools/list`. Restart the server and
